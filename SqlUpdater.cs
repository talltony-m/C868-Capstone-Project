...
namespace C868
{
    class SqlUpdater
    {
        ...
        public static string conString = "server=3.227.166.251 ;database=U04yKm ;uid=U04yKm ;password=53688383395; CharSet=utf8; pooling=false; convert zero datetime = True; ";
        ...
        public static DataTable FirstCal(string filter, bool week)
        {
            ...
            conn.Open();
            string query = week ? $"SELECT (select customerName from customer where customerId = appointment.customerId) as 'Team',  customerId as 'Team Id', type as 'Type', start as 'Start Time', end as 'End Time' FROM appointment where start < '{filter}' and end < '{filter}'  and end > now() order by start;"
                : $"SELECT  (select customerName from customer where customerId = appointment.customerId) as 'Team', customerId as 'Team Id', type as 'Type', start as 'Start Time', end as 'End Time' FROM appointment where start < '{filter}' and end < '{filter}' and end > now() order by start;";
        ...
        public static DataTable Schedule(string id)
        {
            ...
            MySqlConnection conn = new MySqlConnection(conString);
            conn.Open();
            string query = $"SELECT (select customerName from customer where customerId = appointment.customerId) as 'Team',  type as 'Type', start as 'Start Time', end as 'End Time', createdBy as 'Created By' FROM appointment order by start;";
            MySqlCommand cmd = new MySqlCommand(query, conn);
            DataTable dt = new DataTable();
            dt.Load(cmd.ExecuteReader());
        ...
        public static int CreateSchedule(string table, int teamId, string st, string et, string type, string title, string description, string location, string url, string contact, string timestamp, string userName, int userId)
        {
            int schId = CreateID(table);
            string schInsert;
            if (userId == 0)
            {

                schInsert = $"INSERT INTO {table} (appointmentId, customerId,  start, end, type, title, description, location, url, contact, createDate, createdBy, lastUpdate, lastUpdateBy)" +
                $" VALUES ('{schId}', '{teamId}', '{st}', '{et}', '{type}', '{title}', '{description}', '{location}', '{url}', '{contact}', '{timestamp}', '{userName}', '{timestamp}', '{userName}')";

            }
            else
            {
                schInsert = $"INSERT INTO {table} (appointmentId, customerId,  start, end, type, title, description, location, url, contact, userId, createDate, createdBy, lastUpdate, lastUpdateBy)" +
                $" VALUES ('{schId}', '{teamId}', '{st}', '{et}', '{type}', '{title}', '{description}', '{location}', '{url}', '{contact}', '{userId}', '{timestamp}', '{userName}', '{timestamp}', '{userName}')";
            }

            MySqlConnection c = new MySqlConnection(conString);
            c.Open();
            MySqlCommand cmd = new MySqlCommand(schInsert, c);
            cmd.ExecuteNonQuery();
